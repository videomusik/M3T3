...
//    if(!serialBusy) sendControlChange((byte)0, (byte)knobMidiCC[knob], (byte)knobValue[knob]);
    output.sendController((byte)0, (byte)knobMidiCC[knob], (byte)knobValue[knob]);
...
void semi1(int val)
{
  ...
  if(knobValue[knob] != val) {
    ...
    output.sendController((byte)0, (byte)knobMidiCC[knob], (byte)semi);
  }
}
...
void wave1(int val)
{
  ...
  if(knobValue[knob] != val) {
    ...
    output.sendController((byte)0, (byte)knobMidiCC[knob], (byte)(knobValue[knob]*8));
  }
}
